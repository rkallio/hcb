...
class Database(ABC):
    ...
    @abstractmethod
    def get_station_with_departures_and_arrivals(self, station_id):
        return NotImplemented
...
SELECT station.fid, station.english_name, station.finnish_address, statistics.returns, statistics.departures, station.x, station.y
FROM station
JOIN statistics
ON station.id = statistics.id;
        """
        with self.connection.cursor() as cursor:
            cursor.execute(SQL, (station_id,))
            value = cursor.fetchone()
        return StationRecordPlus._make(value)
...
def station(station_id):
    ...
    station = db.get_station_with_departures_and_arrivals(station_id)
    return render_template(
        'station.html.j2',
        station=station,
    )
...
element.style = 'height:min(400px);'
