...
def journeys():
    page_size = 20
    query = "SELECT * FROM journey LIMIT ? OFFSET ?"
    # page indexing starts at 1
    page = max(1, int(request.args.get('page', '1')))
    offset = (page - 1) * page_size
    with sqlite3.connect(os.environ['SQLITE_DB']) as db:
        resp = db.execute(query, (page_size, offset,))
    journeys = map(journey.JourneyRecord._make, resp.fetchall())
