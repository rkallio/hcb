...
It started life as a Python program that interfaced with a SQLite3 database and rendered HTML for clients to consume.  Due to the project requirements, the app was separated in to three components: a PostgreSQL database, a "backend", and a "frontend" server.

The backend is implemented in Python using Flask and flask_restful.  It consumes and produces HTTP messages.  The data source used is a PostgreSQL database.

The frontend is implemented in Python using Flask.  It interfaces between a client and the backend.
...
Some tests implemented using robot framework.
