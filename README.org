...
This is the [[#data-import][Data import]] job.  It is untarred to =hcb-data-import= directory.  There is a Dockerfile that you can build and run.  This is a bit of a memory hog.  2GB of system memory doesn't seem to be enough.  If you get a =Killed= message, increase memory allocated to Docker to 4096MB.
...
The backend is untarred into hcb-backend.  Again, there is a Dockerfile and a sample env provided.  Here's how you would build the container and run it:
...
The frontend is untarred to =hcb-frontend=.  Once again, the directory contains a Dockerfile.  Build and run it.
