...
We want to get the count of departures and returns associated with a station:
...
SELECT station.fid, station.english_name, station.finnish_address, returns, departures
FROM station
JOIN statistics
...
#+RESULTS:
| fid | english_name | finnish_address   | returns | departures |
|-----+--------------+-------------------+---------+------------|
|   1 | Hanasaari    | Hanasaarenranta 1 |    5072 |       4930 |
...
StationRecordPlus = namedtuple(
    'StationRecordPlus',
    'fid,english_name,finnish_address,returns,departures,x,y',
)
...
class PostgreDB(Database):
    ...
    def get_station_statistics(self, station_id):
        total_journeys = self._total_journeys_from_and_to_station(station_id)
        avg_distances = self._avg_distances_from_and_to_station(station_id)
        top_return_stations = self._top_return_stations(station_id)
        top_departure_stations = self._top_departure_stations(station_id)
    ...
    def _total_journeys_from_and_to_station(self, station_id):
        SQL = """
WITH station AS (
    SELECT *
    FROM station
...
ON station.id = statistics.id;
    ...
    def _avg_distances_from_and_to_station(self, station_id):
        SQL = """
WITH station AS (
    SELECT *
    FROM station
...
ON station.id = statistics.id;
    ...
    def _top_return_stations(self, station_id):
        SQL = """
WITH our_station AS (
     SELECT id
     FROM station
...
LIMIT 5;
    ...
    def _top_departure_stations(self, station_id):
        SQL = """
WITH our_station AS (
     SELECT id
     FROM station
...
def station(station_id):
    ...
    statistics = db.get_station_statistics(station_id)
    print(statistics)
    return render_template(
        'station.html.j2',
        station=station,
      ...
      <div class="col d-flex justify-content-center">
        <ul class="pagination">
          <li class="page-item">
            <a
              class="page-link"
              href="{{ url_for('journeys', page=previous_page) }}">
              Previous
            </a>
          </li>
          ...
          <li class="page-item">
            <a class="page-link"
               href="{{ url_for('journeys', page=next_page) }}">
               Next
            </a>
          </li>
        </ul>
      ...
      <div class="col d-flex justify-content-center">
        <ul class="pagination">
          <li class="page-item">
            <a
              class="page-link"
              href="{{ url_for('stations', page=previous_page) }}">
              Previous
            </a>
          </li>
          <li class="page-item active">
          ...
          <li class="page-item">
            <a class="page-link"
               href="{{ url_for('stations', page=next_page) }}">
               Next
            </a>
          </li>
        </ul>
        ...
        <div class="row">
          <div class="col">
            ...
            <table class="table">
              ...
              <tbody>
                <tr>
                  <td>{{ statistics.average_departure_distance }}</td>
                  <td>{{ statistics.average_return_distance }}</td>
                </tr>
              </tbody>
            </table>
          </div>
