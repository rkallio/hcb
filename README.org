...
On App Engine requests to =/stations/<id>= take ~4.5 seconds to complete, because the SQL queries are heavy and the VM running Postgres has no SSD/is low powered.  Figure out a better DB layout or precompute these values or.

** Rename path =/station/<id>= to =/stations/<id>= in frontend.
...
[[https://robotframework-browser.org][Robot Framework Browser]] is used for testing the frontend.  For running the the tests, you must have a local install of [[https://www.python.org/downloads/][Python]], [[https://nodejs.org/en/download][NodeJS]] and npm.  You also need to have a working instance of the app running somewhere that is accessible.
...
Here's a resource file for the tests.  You may want to point the =SERVER= variable somewhere else, like to [[https://hcb.rkll.net]], if you have not set up the app on your local machine.
...
These are tests for the homepage.
...
#+header: :tangle hcb-frontned/tests/stations.robot
#+begin_src robot
,*** Settings ***
...
{% macro _header_link(column_name, column_readable_name, direction, sort_icon, search_term) %}
  <a href="{{ url_for('journeys', page=1, search=search_term, order_by=column_name, direction=direction) }}">{{ column_readable_name }}</a>
  <i class="{{ sort_icon }}"></i>
{% endmacro %}

{% macro _table_header_element(column_name, column_readable_name, order_by, data_type, direction, search_term) %}
  ...
  <th>
    {% if order_by == column_name and direction == 'ascending' %}
      {{ _header_link(column_name, column_readable_name, 'descending', icon, search_term) }}
    {% else %}
      {{ _header_link(column_name, column_readable_name, 'ascending', icon, search_term) }}
    {% endif %}
  </th>
{% endmacro %}
      ...
      <div class="col">
        <div class="table-responsive">
          <table class="table table-striped table-hover">
            <thead>
             <tr>
               {{ _table_header_element('departure_station', 'Departure Station', order_by, 'alpha', direction, search_term) }}
               {{ _table_header_element('return_station', 'Return Station', order_by, 'alpha', direction, search_term) }}
               {{ _table_header_element('distance', 'Covered distance (km)', order_by, 'numeric', direction, search_term) }}
               {{ _table_header_element('duration', 'Duration (m)', order_by, 'numeric', direction, search_term) }}
              </tr>
            </thead>
            <tbody>
